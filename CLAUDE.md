...
**Batching**: Classes are grouped by source directory first, then capped by `--batch-size` (default 6) and a real token ceiling (default 4,000 input tokens). Token counts come from the Anthropic SDK's `count_tokens` endpoint — never a character-count heuristic or tiktoken. The ceiling is per request: the constant system prompt's tokens are counted once and reserved from it, each multi-class batch's assembled text is re-counted once to verify it fits (bisect-resplit on the rare overshoot), and structured-output schema/tool overhead is not yet counted (issue #20).
