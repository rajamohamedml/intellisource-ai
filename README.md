...
- **The per-batch token ceiling does not yet cover structured-output schema/tool-definition overhead** (tracked in issue #20). The constant system prompt is counted once and reserved, and each multi-class batch's assembled text is re-counted to verify it fits, but the schema LangChain attaches to each request is not. A single method that alone exceeds the ceiling also cannot be split further and is dispatched oversized.
